...
export const LuxuryOrderReceiptModal: React.FC<LuxuryOrderReceiptModalProps> = ({
  ...
  const payload: OrderReceiptPayload = {
    ...
    turnaroundSpeed: details.urgency || 'Standard Delivery (24-48h)',
    deadline: details.urgency || 'Standard Delivery',
    basePrice: details.discountApplied ? `${details.currency} ${(details.amount * 2).toLocaleString()}` : `${details.currency} ${details.amount.toLocaleString()}`,
    rushFee: details.urgency?.includes('Express') || details.urgency?.includes('Urgent') || details.urgency?.includes('Priority') ? 'Rush Multiplier Included' : undefined,
